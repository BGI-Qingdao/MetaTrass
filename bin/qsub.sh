...
do
        ...
        if [ $depth == 'X10' ];
                then
                record=`qsub -cwd -l vf=20g,num_proc=1 -P P18Z19700N0076 -q st.q -binding linear:1 $line`;
                qid=`echo $record|awk '{print $3}'`;
                qsub -cwd -hold_jid $qid -l vf=50g,num_proc=20 -P P18Z19700N0076 -q st.q -binding linear:20 $outdir/step0_commandsh/step3.1.batch_supernova_assembly_sh/step3.1_X10_$tid\_$strategy;
                qsub -cwd -hold_jid $qid -l vf=20g,num_proc=16 -P P18Z19700N0076 -q st.q -binding linear:16 $outdir/step0_commandsh/step3.2.batch_idba_assembly_sh/step3.2_X10_$tid\_$strategy;
        elif [ $depth == 'X300' ];
                then
                record=`qsub -cwd -l vf=50g,num_proc=1 -P P18Z19700N0076 -q st.q -binding linear:1 $line` ;
                qid=`echo $record|awk '{print $3}'`;
                qsub -cwd -hold_jid $qid -l vf=90g,num_proc=20 -P P18Z19700N0076 -q st.q -binding linear:20 $outdir/step0_commandsh/step3.1.batch_supernova_assembly_sh/step3.1_X300_$tid\_$strategy;
